...
- ✅ **단계 쪼개기** - `statement()` 로직을 두 단계로 나누어 첫 단계는 `statement()`에 필요한 데이터를 처리하고, 다음 단계에서 앞서 처리한 결과를 텍스트나 HTML로 표현한다.
  ...
  ```javascript
  // 🔧 단계 쪼개기를 위해서 두 번째 단계가 될 코드들을 함수 추출하기로 뽑아낸다.
  function statement(invoice, plays) {
...
- ✅ `renderPlainText()`의 매개변수 `invoice`, `plays`를 중간 데이터 구조로 옮기면 **계산 관련 코드를 `statement()` 함수로 모으고 `renderPlainText()`는 `data` 매개변수로 전달된 데이터만 처리할 수 있다.**
  ...
  function statement(invoice, plays) {
    ...
    // 🔧 `renderPlainText()`의 인수 `invoice` 제거 및 중간 데이터 구조를 인수로 전달
    return renderPlainText(statementData, plays);
  }

  // 🔧 `renderPlainText()`의 매개변수 `invoice` 제거
  function renderPlainText(data, plays) {
    // 🔧 고객 데이터를 중간 데이터인 `data`로 사용
