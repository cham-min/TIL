...
TypeScript는 함수의 매개변수와 반환값의 타입을 지정할 수 있다.

```typescript
/**
 * 1. 매개변수 타입 표기
 * - 잘못된 타입의 인자를 전달할 경우 컴파일 에러가 발생한다.
 * - 필수 매개변수만큼 인자가 전달되지 않을 경우 에러가 발생한다.
 * - 필수 매개변수는 선택적 매개변수보다 먼저 작성해야 한다.
 * - 선택적 매개변수를 사용할 때에는 undefined 타입으로도 추론되기 때문에 타입 가드를 잘 활용해야 한다.
 */
function greet(name: string) {
  // const greet = (name: string) => { ... }과 동일
  console.log(`안녕하세요 ${name}님`);
}

greet(30); // ❌ Argument of type 'number' is not assignable to parameter of type 'string'.

/**
 * 2. 반환 타입 표기
 * - 반환 타입은 타입 추론되기 때문에 필요하지 않지만, 선호도에 따라서 문서화 목적 및 우발적 변경을 방지하기 위해서 작성하곤 한다.
 */
...
/**
 * 3. 함수 타입 표현식
 * - 함수의 매개변수와 반환 타입을 명시적으로 정의하는 것이다.
 * - 타입 별칭을 활용하여 재사용 가능한 함수 타입 정의가 가능하다.
 */
...
/**
 * 4. 호출 시그니처
 * - 함수가 어떻게 호출될 수 있는지를 객체 타입 내에서 함수의 매개변수와 반환 타입을 명시할 때 사용한다.
 */
type Calculator2 = {
  (a: number, b: number): number;
};

const add: Calculator2 = (a, b) => a + b;
```

<br>

익명 함수의 경우 함수가 어떻게 호출되는지 알아내고 매개 변수에 자동으로 타입을 부여한다. 아래 코드를 살펴보면 함수에 대한 문맥적 타이핑(contextual typing)으로 매개변수 `name`은 `string`으로 타입 추론된다.
...
/**
 * 2. 인터페이스
 * - 객체의 구조를 정의하는 방법으로 속성과 메서드의 타입을 명시할 수 있다.
 * - 기존 타입 별칭과 마찬가지로 선택적 속성, 읽기 전용 속성, 호출 시그니처, 인덱스 시그니처 정의가 가능하다
 * - 객체 구조에 정의에 특화되어 `extends` 키워드를 사용하여 인터페이스를 확장하여 새로운 인터페이스를 만들 수 있다.
 * - 호출 시그니처 형태로 함수 오버로드 작성이 가능하다.
 */
interface Person {
  name: string;
  age: number;
...
타입을 확장할 때 기존 속성의 타입을 `extends`를 사용하여 구체적으로 재정의할 수 있다. 단, 확장타입은 기본타입의 서브타입이어야 한다. 따라서 속성의 타입을 재정의할 때에도 서브타입으로만 재정의가 가능하다.
...
/**
 * Interface 병합
 * - Interface 확장과는 다르게 서브타입일 경우 호환되지 않고 동일한 타입으로만 병합이 가능하다.
 * - 라이브러리 등 외부 모듈의 타입 정의를 보강할 때 사용할 수 있다.
 */
interface Fries {
  salt: boolean;
}
